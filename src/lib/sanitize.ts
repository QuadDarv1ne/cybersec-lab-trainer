 ...
 * Uses iterative strip→decode→strip cycle to prevent double-encoded XSS attacks.
 */
...
function decodeEntities(input: string): string {
  return input
    ...
    .replace(/&([a-zA-Z]+);/g, (_, name) => {
      ...
      return namedEntities[name] || `&${name};`;
    });
}
...
export function sanitizeNoteContent(raw: string): string {
  ...
  // Server-side (Node.js): DOMParser is not available, use regex-based sanitization.
  // Apply strip→decode→strip cycle to prevent double-encoded XSS:
  //   &amp;lt;script&amp;gt; → &lt;script&gt; → <script> (stripped on second pass)
  if (typeof DOMParser === 'undefined') {
    let result = stripTags(raw); // pass 1: strip any literal tags
    result = decodeEntities(result); // decode entities
    result = stripTags(result); // pass 2: strip tags revealed by decoding
    return result.trim();
  }
